...
function init_sidebar() {
    ...
    $SIDEBAR_MENU.find("a").on("click", function (a) {
        ...
        t.is(".active") ? (t.removeClass("active active-sm"), $("ul:first", t).slideUp(function () {
            console.log("setContentHeight => 1"), e()
        }
            ...
            console.log("setContentHeight => 2"), e()
        }
        ))
    }
    ),
        $MENU_TOGGLE.on("click", function () {
            console.log("clicked - menu toggle"), $BODY.hasClass("nav-md") ? ($SIDEBAR_MENU.find("li.active ul").hide(), $SIDEBAR_MENU.find("li.active").addClass("active-sm").removeClass("active")) : ($SIDEBAR_MENU.find("li.active-sm ul").show(), $SIDEBAR_MENU.find("li.active-sm").addClass("active").removeClass("active-sm")), $BODY.toggleClass("nav-md nav-sm"), console.log("setContentHeight => 3"), e(), $(".dataTable").each(function () {
        ...
        $SIDEBAR_MENU.find("a").filter(function () {
            ...
            console.log("setContentHeight => 4"), e()
        }
        ).parent().addClass("active"),
        console.log("setContentHeight => 6"),
        e(),
...
function GraficoRreceitaDespesa(_url, _target) {
    let e = $(_target);
    let labels = [], dataDespesa = [], dataReceita = [];
    let totalDespesas = 0;
    let totalReceitas = 0;
    ...
    var jsonData = $.ajax({
        method: "POST",
        url: _url,
        data: { unidade: $("#unidade").val(), periodo: $("#reservation").val(), _token: $("input[name=_token]").val() },
        dataType: 'json',
    }).done(function (results) {
        results.forEach(function (item) {
            let mesAno = item.mes.split('/');
            if (labels.indexOf(GetMes(mesAno[0]) + '/' + mesAno[1]) === -1) {
                labels.push(GetMes(mesAno[0]) + '/' + mesAno[1]);
                /*pega as receitas do mês/ano*/
                let bInsereZero = true;
                results.forEach(function (find) {
                    if (find.mes == item.mes && find.Tipo === "RECEITA") {
                        dataReceita.push(find.total.replace(",", ""));
                        totalReceitas += parseFloat(find.total.replace(",", ""));
                        bInsereZero = false;
                    }
                });
                if (bInsereZero) {
                ...
                /*pega as despesas do mês/ano*/
                bInsereZero = true;
                let valor = 0;
                results.forEach(function (find) {
                    if (find.mes == item.mes && find.Tipo === "DESPESA") {
                        valor = (find.total.replace(",", "")) * -1;
                        dataDespesa.push(valor.toFixed(2).toString().replace(",", ""));
                        totalDespesas += parseFloat(valor.toFixed(2).toString().replace(",", ""));
                        bInsereZero = false;
                    }
                });
                if (bInsereZero) {
        ...
        dataDespesa.push(totalDespesas);
        dataReceita.push(totalReceitas);
                    ...
                    tooltips: {
                        callbacks: {
                            label: function (tooltipItem, data) {
                                ...
                                if (label === "DESPESAS") {
                                    label += ": - R$ " + (((Math.round(tooltipItem.yLabel * 100) / 100)).toLocaleString('pt-BR'));
                                } else {
                                    label += ": R$ " + (((Math.round(tooltipItem.yLabel * 100) / 100)).toLocaleString('pt-BR'));
                                }
...
$(document).ready(function () {
    ...
    $("#send-excel-fat-analitico").click(function () {
        $.ajax({
            method: "GET",
            url: '/GetExcel',
            data: { unidade: $("#unidade").val(), periodo: $("#reservation").val(), _token: $("input[name=_token]").val() },
...
function init_flot_chart() {
    if (void 0 !== $.plot) {
        ...
        for (var e = [[gd(2012, 1, 1), 17], [gd(2012, 1, 2), 74], [gd(2012, 1, 3), 6], [gd(2012, 1, 4), 39], [gd(2012, 1, 5), 20], [gd(2012, 1, 6), 85], [gd(2012, 1, 7), 7]], a = [[gd(2012, 1, 1), 82], [gd(2012, 1, 2), 23], [gd(2012, 1, 3), 66], [gd(2012, 1, 4), 9], [gd(2012, 1, 5), 119], [gd(2012, 1, 6), 6], [gd(2012, 1, 7), 9]], t = [], o = 0;
            o < 30;
            o++)t.push([new Date(Date.today().add(o).days()).getTime(), randNum() + o + o + 10]);
        var n = {
            grid: {
        ...
        $('[data-toggle="tooltip"]').tooltip(),
            ...
            $(".docs-buttons").on("click", "[data-method]", function () {
                ...
                if (!n.prop("disabled") && !n.hasClass("disabled") && e.data("cropper") && i.method) {
                    if (void 0 !== (i = $.extend({}
                        , i)).target && (t = $(i.target), void 0 === i.option)) try {
                    ...
                    switch (o = e.cropper(i.method, i.option, i.secondOption), i.method) {
                        case "scaleX": case "scaleY": $(this).data("option", -i.option);
                            break;
                        case "getCroppedCanvas": o && ($("#getCroppedCanvasModal").modal().find(".modal-body").html(o), a.hasClass("disabled") || a.attr("href", o.toDataURL()))
                    }
...
function filtroMatricula() {
    ...
    $('#filter').click(function () {
        ...
        var reservation2 = $('#reservation').val();
        if (reservation != '' && reservation2 != '') {
            $('#matriculados').DataTable().destroy();
            load_data(reservation, reservation2);
        }
        else {
...
function init_charts() {
    if (console.log("run_charts  typeof [" + typeof Chart + "]"),
        ...
        $("#buscar-grafico-receita-despesa").click(function () {
            GraficoRreceitaDespesa('/faturamento-receita-despesa', '#barChart');
        });


        $("#buscar-grafico-aluno-status").click(function () {
            let e = document.getElementById("barChart");
            let labels = [], dataInativos = [], dataAtivos = [];

            var jsonData = $.ajax({
                method: "POST",
                url: '/alunos-ativos-inativos',
                data: { unidade: $("#unidade").val(), periodo: $("#reservation").val(), _token: $("input[name=_token]").val() },
                dataType: 'json',
            }).done(function (results) {
                ...
                $("#chart").css({ display: "block" });
                results.forEach(function (item) {
                    if (item.Status == "Estudando") {
                        dataAtivos.push(item.total);
                    }
                    else {
                        dataInativos.push(item.total);
                    }
                });
                var tempData = {
                    labels: labels,
        ...
        if ($("#lineChart").length) {
            ...
            var jsonData = $.ajax({
                ...
                //var myLineChart = new Chart(e).Line(tempData);
        ...
        if ($("#lineChartMatriculasDia").length) {
            let e = document.getElementById("lineChartMatriculasDia");
            let labels = [], data = [];
        ...
        if ($("#mybarChart").length > 0) {
            GraficoRreceitaDespesa('/receitas-despesas', '#mybarChart');
        }
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    let a = {
                        ...
                        datasets: [{
                            data: [data.matricula, data.negociado, data.desistente],
                            backgroundColor: ["#26B99A", "#3498DB", "#455C73"],
                            hoverBackgroundColor: ["#36CAAB", "#49A9EA", "#34495E"]
                        }
                        ]
                    };
                    new Chart(e,
    ...
    if (void 0 !== $.fn.fullCalendar) {
        ...
        var e,
            ...
            i = $("#calendar").fullCalendar({
                    ...
                    $("#fc_create").click(),
                        ...
                        $(".antosubmit").on("click", function () {
                            ...
                            return t && (ended = t), $("#event_type").val(), a && i.fullCalendar("renderEvent", {
                                title: a, start: e, end: t, allDay: o
                            }
                ...
                editable: !0,
                events: [{
                    title: "All Day Event", start: new Date(n, o, 1)
                }
                    ,
                {
                    title: "Long Event", start: new Date(n, o, t - 5), end: new Date(n, o, t - 2)
                }
                    ,
                {
                    title: "Meeting", start: new Date(n, o, t, 10, 30), allDay: !1
                }
                    ,
                {
                    title: "Lunch", start: new Date(n, o, t + 14, 12, 0), end: new Date(n, o, t, 14, 0), allDay: !1
                }
                    ,
                {
                    title: "Birthday Party", start: new Date(n, o, t + 1, 19, 0), end: new Date(n, o, t + 1, 22, 30), allDay: !1
                }
                    ,
                {
                    title: "Click for Google", start: new Date(n, o, 28), end: new Date(n, o, 29), url: "http://google.com/"
                }
                ]
            }
            )
...
$("#ocultar-filtros").click(function () {
